...
/*
 *  ターゲット依存モジュール（FRDM-MCXN947 + MCUXpresso SDK用）
 *
 *  クロック・ピンの初期化は SDK 側（startup_MCXN947_cm33_core0.S
 *  → SystemInit → main() の BOARD_InitBootPins/BOARD_InitBootClocks）が行う．
 *  MCXN947 は内蔵フラッシュ実行のため FlexSPI/XIP 設定は不要．
...
target_initialize(void)
{
	...
	/*
	 *  システムログの低レベル出力用に SIO（LPUART4）を初期化する．
