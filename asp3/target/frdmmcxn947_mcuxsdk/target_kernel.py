...
#
#     パス2のターゲット依存テンプレート（FRDM-MCXN947 / MCUXpresso SDK 用）
#
#  arm_m 共通テンプレート（core_kernel.py）は使わず，kernel/kernel.py を直接
#  インクルードしてベクタテーブルをここで生成する（FSP と同方針）．理由：
#  リセットエントリと初期スタックポインタを SDK の startup_MCXN947_cm33_core0.S
#  （Reset_Handler / __StackTop）に向けるため．core_kernel.py は _kernel_start /
...
#  置く必要がある（ARMv8-M）．SDK のリンカスクリプトに .vector の
#  配置規則は無く orphan section（4byte整列）となるため，整列違反だと
#  VTOR 下位ビットが切り捨てられ全ベクタがずれて即死する（STM32統合のNUCLEO-H533REで実害確認済みの論点）．
#
 ...
 *  ベクターテーブル
 *
 *  リセットエントリ・初期スタックポインタは SDK の startup/リンカスクリプトから引く．
 *  Reset_Handler は target_kernel.h，__StackTop は target_kernel_impl.h で
