...
/*
 *  タイマドライバ（FRDM-MCXN947 + MCUXpresso SDK用）
 *
 ...
 *  使用と同じ位置付け）．
 *
 *  カウンタクロックは FRO_HF（PLL150M ブートで 48MHz 起動済み）を CTIMER の
 *  クロックディバイダで 48分周し，正確に 1.000000MHz（ppm 誤差なし）にする．
 *  ディバイダの HALT 解除（CLOCK_SetClkDiv）を忘れると CTIMER_Init で
 *  バスストールするので注意（target_timer.c 参照）．
 */
...
/*
 *  タイマ割込みハンドラ登録のための定数（CTIMER0_IRQn = 31）
 */
